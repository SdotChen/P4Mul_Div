    ...
    inout ingress_intrinsic_metadata_for_tm_t        ig_tm_md)
{
    ...
    @stage(0)
    table get_rnd_val1_t {
        actions = { get_rnd_val1;}
        default_action = get_rnd_val1();
        size = 1;
    }
    ...
    @stage(0)
    table get_rnd_val2_t {
        actions = { get_rnd_val2;}
        default_action = get_rnd_val2();
        size = 1;
    }
    ...
    Register<bit<32>,bit<16>>(65536) small;
    RegisterAction<bit<32>,bit<16>,bit<1>>(small) _save_small = {
    ...
    action save_small() {
        _save_small.execute(meta.seq);
    }

    @stage(2)
    table save_small_t {
        actions = { save_small;}
        default_action = save_small();
    }

    Register<bit<32>,bit<16>>(65536) big;
    RegisterAction<bit<32>,bit<16>,bit<1>>(big) _save_big = {
        void apply(inout bit<32> reg_data) {
            reg_data = meta.big;
        }
    };
    ...
    action save_big() {
        _save_big.execute(meta.seq);
    }

    @stage(2)
    table save_big_t {
        actions = { save_big;}
        default_action = save_big();
        size = 1;
    }
    ...
    Register<bit<32>,bit<16>>(65536) cal_res_lo;
    RegisterAction<bit<32>,bit<16>,bit<1>>(cal_res_lo) _save_cal_res_lo = {
        void apply(inout bit<32> reg_data){
    ...
    action save_cal_res_lo() {
        _save_cal_res_lo.execute(hdr.mul.seq);
    }
    ...
    Register<bit<32>,bit<16>>(65536) cal_res_hi;
    RegisterAction<bit<32>,bit<16>,bit<1>>(cal_res_hi) _save_cal_res_hi = {
        void apply(inout bit<32> reg_data){
            if(hdr.mul.rest_coeff == 0){
    ...
    action save_cal_res_hi() {
        _save_cal_res_hi.execute(hdr.mul.seq);
    }
    ...
    apply {
        if(ig_intr_md.ingress_port != RECIRC_PORT){
            ...
            get_rnd_val1_t.apply();     // stage 0
            get_rnd_val2_t.apply();     // stage 0

            get_seq_t.apply();          // stage 1

            get_big_small_t.apply();    // stage 1
            save_small_t.apply();       // stage 2
            save_big_t.apply();         // stage 2

            add_mul_hdr_t.apply();      // stage 2
        }
        route_t.apply();                // stage 5
        save_cal_res_lo_t.apply();      // stage 10
        save_cal_res_hi_t.apply();      // stage 11
    }
}
